...
class ParserTest {
	...
	private static final String example = "pbga (66)\n" + 
			"xhth (57)\n" + 
			"ebii (61)\n" + 
			"havc (66)\n" + 
			"ktlj (57)\n" + 
	...
	void testLeaf() {
		...
		assertEquals("pbga", program.getName());
		assertEquals(66, program.getWeight());
		assertTrue(program.getChildrenNames().isEmpty());
	}

	@Test
	void testNode() {
		String input = "fwft (72) -> ktlj, cntj, xhth";
		
		Program program = parse(input);
		
		assertEquals("fwft", program.getName());
		assertEquals(72, program.getWeight());
		assertTrue(program.getChildrenNames().contains("ktlj"));
		assertTrue(program.getChildrenNames().contains("cntj"));
		assertTrue(program.getChildrenNames().contains("xhth"));
	}
	...
	void testFindChildren() {
		...
		List<String> result = findHeads(input);
		
		assertEquals(1, result.size());
		assertEquals("tknk", result.get(0));
	}

	@Test
	void testFindChildrenPuzzleInput() {
		String input = puzzleInput;
		
		List<String> result = findHeads(input);
		
		assertEquals(1, result.size());
		assertEquals("ahnofa", result.get(0));
	}
	
	@Test
	void testRecursiveWeightLeaf() {
		String input = example;
		
		Map<String, Program> programs = parseAllPrograms(input).stream()//
				.collect(Collectors.toMap(p -> p.getName(), p -> p));
		
		int result = getRecursiveWeight(programs, programs.get("gyxo"));
		
		assertEquals(61, result);
	}
	
	@Test
	void testRecursiveWeightFinalNode() {
		String input = example;
		
		Map<String, Program> programs = parseAllPrograms(input).stream()//
				.collect(Collectors.toMap(p -> p.getName(), p -> p));
		
		int result = getRecursiveWeight(programs, programs.get("ugml"));
		
		assertEquals(251, result);
	}
	
	@Test
	void testRecursiveWeightNode() {
		String input = example;
		
		Map<String, Program> programs = parseAllPrograms(input).stream()//
				.collect(Collectors.toMap(p -> p.getName(), p -> p));
		
		int result = getRecursiveWeight(programs, programs.get("tknk"));
		
		assertEquals(251 + 243 + 243 + 41, result);
	}
	
	@Test
	void testWellBalancedLeaf() {
		String input = example;
		
		Map<String, Program> programs = parseAllPrograms(input).stream()//
				.collect(Collectors.toMap(p -> p.getName(), p -> p));
		
		boolean result = isWellBalanced(programs, programs.get("gyxo"));
		
		assertEquals(true, result);
	}
	
	@Test
	void testWellBalancedFinalNode() {
		String input = example;
		
		Map<String, Program> programs = parseAllPrograms(input).stream()//
				.collect(Collectors.toMap(p -> p.getName(), p -> p));
		
		boolean result = isWellBalanced(programs, programs.get("ugml"));
		
		assertEquals(true, result);
	}
	
	@Test
	void testNotWellBalancedFinalNode() {
		String input = example;
		
		Map<String, Program> programs = parseAllPrograms(input).stream()//
				.collect(Collectors.toMap(p -> p.getName(), p -> p));
		
		boolean result = isWellBalanced(programs, programs.get("tknk"));
		
		assertEquals(false, result);
	}
	
	@Test
	void testFindNotWellBalancedNode() {
		String input = example;
		
		Map<String, Program> programs = parseAllPrograms(input).stream()//
				.collect(Collectors.toMap(p -> p.getName(), p -> p));
		
		List<Program> result = programs.values().stream()//
			.filter(p -> !isWellBalanced(programs, p))//
			.collect(Collectors.toList());
		
		assertEquals(1, result.size());
		assertEquals("tknk", result.get(0).getName());
	}
	
	@Test
	void testFindNotWellBalancedPuzzle() {
		String input = puzzleInput;
		...
		Program father = programs.get("bvrxeo");
		
		father.getChildrenNames().stream()//
			.map(name -> programs.get(name))//
			.forEach(p -> System.out.println("Poids pour " + p.getName() + " : " + getRecursiveWeight(programs, p)));
		
		assertEquals(1, result.size());
		assertEquals("tknk", result.get(0).getName());
	}
	
	private boolean isWellBalanced(Map<String, Program> programs, Program program) {
		List<Program> children = program.getChildrenNames().stream()//
			.map(name -> programs.get(name))//
			.collect(Collectors.toList());
		...
		int reference = getRecursiveWeight(programs, children.get(0));
		
		Optional<Program> erreur = children.stream().filter(p -> getRecursiveWeight(programs, p) != reference).findFirst();
		
		erreur.ifPresent(p -> System.out.println("Incohérence en " + program.getName() + " : " + reference + " et " + getRecursiveWeight(programs, p)));
		
		return !erreur.isPresent();
	}
	
	private int getRecursiveWeight(Map<String, Program> programs, Program program) {
		int childrensWeight = program.getChildrenNames().stream()//
			.map(name -> programs.get(name))//
			.map(p -> getRecursiveWeight(programs, p))//
			.collect(Collectors.summingInt(Integer::intValue));
		
		return program.getWeight() + childrensWeight;
	}
	
	private List<String> findHeads(String input) {
		List<Program> programes = parseAllPrograms(input);
		Set<String> programNames = programes.stream()//
				.map(Program::getName)//
				.collect(toSet());
		Set<String> childrenNames = programes.stream()//
				.map(Program::getChildrenNames)//
				.flatMap(List::stream)//
				.collect(toSet());
		programNames.removeAll(childrenNames);
		List<String> result = new ArrayList<>(programNames);
		return result;
	}
	
	private List<Program> parseAllPrograms(String input) {
	...
	private Program parse(String input) {
        ...
        Integer weigth = Integer.valueOf(matcher.group(2));
        String children = matcher.group(4);
        
        if(children == null) {
    		return new Program(name, emptyList(), weigth);
        }
        
        List<String> childrenNames = Arrays.asList(children.split(", "));
        
		return new Program(name, childrenNames, weigth);
	}

	
	private String puzzleInput = "jovejmr (40)\n" + 
			"fesmk (24)\n" + 
			"gwhfv (74)\n" + 
...
//			"ltleg (808) -> vnlmjy, bscjk, uycjw, kzuimi, kwlqal, lahieha\n" + 
			"ltleg (802) -> vnlmjy, bscjk, uycjw, kzuimi, kwlqal, lahieha\n" + 
			"cvewsdi (16)\n" + 
			"yiitzs (62)\n" + 
